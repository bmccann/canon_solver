...
class VectorSpaceSearchProblem(SearchProblem):
    def __init__(self, size): self.size, self.start = size, (0,)*size
    def startState(self): return self.start
    def isGoal(self, state): return sum(state) == self.size
    def succAndCost(self, state):
        results = []
        total = sum(state)
        if total == 0:
            results.append(("Correct", (1,)*self.size, self.size))

        for i in range(0, self.size):
            if state[i] == 1: continue
            next = list(state)
            next[i] = 1
            results.append((str(i), tuple(next), 1)) 
        return results

def VectorHeuristic(s):
